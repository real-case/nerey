...
import { widgetRootAttributes } from '../data-attrs';
...
function renderDiv(domProps: Record<string, unknown>): ReactElement {
  return <div {...domProps} />;
}
...
export function WidgetRoot(props: WidgetRootProps): ReactElement {
  const { type, version, slot, status, state, readonly, className, style, render, children } = props;

  const domProps: Record<string, unknown> = {
    ...widgetRootAttributes({ type, version, slot, status, state, readonly }),
    // Conditional so an unset prop leaves no `class=""` / `style=""` on the node and no
    // undefined-valued key for a `render` function to spread onto a foreign component.
    ...(className !== undefined ? { className } : {}),
    ...(style !== undefined ? { style } : {}),
    children,
  };

  return renderWith(render, domProps, renderDiv);
}
